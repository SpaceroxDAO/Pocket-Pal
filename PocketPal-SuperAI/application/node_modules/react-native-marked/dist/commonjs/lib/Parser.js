...
class Parser {
  ...
  _parseToken(token, styles) {
    switch (token.type) {
      case "paragraph":
        {
          const children = this.getNormalizedSiblingNodesForBlockAndInlineTokens(token.tokens, this.styles.text);
      ...
      case "code":
        {
          return this.renderer.code(token.text, token.lang, this.styles.code, this.styles.em);
        }
      case "hr":
        {
      ...
      case "list":
        {
          ...
          const li = token.items.map(item => {
            const children = item.tokens.flatMap(cItem => {
              if (cItem.type === "text") {
                ...
                const listChildren = this.getNormalizedSiblingNodesForBlockAndInlineTokens(childTokens, this.styles.li);
          ...
          return this.renderer.list(token.ordered, li, this.styles.list, this.styles.li, startIndex);
        }
      case "escape":
        {
      ...
      case "link":
        {
          ...
          const href = (0, _url.getValidURL)(this.baseUrl, token.href);
          if (this.hasDuplicateTextChildToken(token)) {
      ...
      case "image":
        {
          return this.renderer.image(token.href, token.text || token.title, this.styles.image);
        }
      case "strong":
        {
          const boldStyle = {
      ...
      case "codespan":
        {
          return this.renderer.codespan((0, _htmlEntities.decode)(token.text), {
      ...
      case "html":
        {
          console.warn("react-native-marked: rendering html from markdown is not supported");
      ...
      case "table":
        {
          const header = token.header.map((row, i) => this._parse(row.tokens, {
            ...(0, _table.getTableColAlignmentStyle)(token.align[i])
          }));
          const rows = token.rows.map(cols => cols.map((col, i) => this._parse(col.tokens, {
            ...(0, _table.getTableColAlignmentStyle)(token.align[i])
          })));
          return this.renderer.table(header, rows, this.styles.table, this.styles.tableRow, this.styles.tableCell);
        }
      case "custom":
        {
          const children = this._parse(token.tokens ?? []);
          return this.renderer.custom(token.identifier, token.raw, children, token.args);
        }
      default:
        {
          return null;
  ...
  getNormalizedSiblingNodesForBlockAndInlineTokens(tokens, textStyle) {
    ...
    for (const t of tokens) {
       ...
       */
      if (t.type === "image" || t.type === "link" && t.tokens && t.tokens[0] && t.tokens[0].type === "image") {
        ...
        if (t.type === "image") {
          ...
          const href = (0, _url.getValidURL)(this.baseUrl, t.href);
          siblingNodes.push(this.renderer.linkImage(href, imageToken.href, imageToken.text || imageToken.title, this.styles.image));
        }
        tokenRenderQueue = [];
        continue;
      }
